...
async def load_gift_pool():
    ...
    try:
        ...
        log.warning(f"⚠️ Не удалось загрузить подарки: {e}")
        gift_pool = []
...
async def transfer_nft(user_id: int, msg_id: int, winner_name: str):
    ...
    functions.payments.TransferStarGift(
        ...
        if "STARGIFT_TRANSFER_TOO_EARLY" in err:
            ...
            log.warning(f"⏳ NFT msg_id={msg_id} заблокирован на {seconds}с ({hours}ч)")
            return "too_early"
...
async def handle_health(request: web.Request):
    gifts_info = [{"gift_id": g["gift_id"], "stars": g["stars"], "is_nft": g["is_nft"], "title": g["title"]} for g in gift_pool]
    total_stars = sum(g.get("stars", 0) for g in gift_pool)
