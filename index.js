...
// Bank webhook config
// ------------------
const BANK_URL    = process.env.BANK_URL || "http://localhost:8080/webhook";
const BANK_SECRET = process.env.BANK_SECRET || "supersecret";

/**
 * Отправляет победителя на банк-бот (userbot),
 * который автоматически пришлёт ему подарок.
 */
async function notifyBank({ userId, username, firstName, gameMode, groupTitle }) {
  try {
    const res = await fetch(BANK_URL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Secret": BANK_SECRET,
      },
      body: JSON.stringify({
        user_id:     userId,
    ...
    if (json.ok) {
      console.log(`🎁 Банк уведомлён о победителе: ${firstName} (${userId})`);
    } else {
      console.warn("⚠️ Банк вернул ошибку:", json);
    }
  } catch (e) {
    console.error("❌ Не удалось связаться с банком:", e.message);
  }
}
...
// --- перебив сообщений ---
const msgPerebivMinutes = new Map();
...
bot.on("message", async (msg) => {
  ...
  if (botEnabled && currentMode === "msgperebiv" && !msg.text.startsWith("/")) {
    ...
    const timeoutId = setTimeout(() => {
      ...
      // в группу
      bot.sendMessage(chatId, `🏆 ПОБЕДИТЕЛЬ!\n${winnerName} написал последним и никто не перебил за ${mins} мин.! 👑\n\n🎁 Банк отправит подарок!`).catch(() => {});

      // уведомляем банк — userbot отправит подарок
      notifyBank({
...
bot.on("dice", async (msg) => {
  ...
  if (currentMode === "slot" && msg.dice.emoji === "🎰") {
    ...
    if (sub === "perebiv") {
      ...
      const timeoutId = setTimeout(() => {
        ...
        mode.delete(chatId);
        perebivMinutes.delete(chatId);
        slotSubMode.delete(chatId);
        slotTrigger.delete(chatId);

        const winnerName = winner.username ? `@${winner.username}` : winner.firstName;
        const winnerLink = winner.username ? `https://t.me/${winner.username}` : `tg://user?id=${winner.userId}`;

        bot.sendMessage(chatId, `🏆 ПОБЕДИТЕЛЬ!\n${winnerName} выбил ${triggerLabel} и никто не перебил за ${mins} мин.! 👑\n\n🎁 Банк отправит подарок!`).catch(() => {});

        // уведомляем банк — userbot пришлёт подарок
        notifyBank({
          userId:     winner.userId,
          username:   winner.username,
          firstName:  winner.firstName,
  ...
  if (currentMode === "cube" && msg.dice.emoji === "🎲" && value === 6) {
    ...
    notifyBank({ userId: user.id, username: user.username, firstName: user.first_name, gameMode: "cube", groupTitle: msg.chat.title });
  }
  ...
  if (currentMode === "basket" && msg.dice.emoji === "🏀" && value === 5) {
    ...
    notifyBank({ userId: user.id, username: user.username, firstName: user.first_name, gameMode: "basket", groupTitle: msg.chat.title });
  }
  ...
  if (currentMode === "darts" && msg.dice.emoji === "🎯" && value === 6) {
    ...
    notifyBank({ userId: user.id, username: user.username, firstName: user.first_name, gameMode: "darts", groupTitle: msg.chat.title });
  }
  ...
  if (currentMode === "bowling" && msg.dice.emoji === "🎳" && value === 6) {
    ...
    notifyBank({ userId: user.id, username: user.username, firstName: user.first_name, gameMode: "bowling", groupTitle: msg.chat.title });
  }
  ...
  if (currentMode === "football" && (msg.dice.emoji === "⚽" || msg.dice.emoji === "⚽️") && value >= 5) {
    ...
    notifyBank({ userId: user.id, username: user.username, firstName: user.first_name, gameMode: "football", groupTitle: msg.chat.title });
  }
});
